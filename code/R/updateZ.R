...
updateZ1 <- function(y, x.beta, zg, prec, tau, mu, taug, g, lambda) {
  nknots <- nrow(tau)
  nt     <- ncol(tau)
  ns     <- nrow(y)
  ...
  if (nknots == 1) {
    ...
    z.update <- z.Rcpp1(taug=taug, tau=tau, y=y, x_beta=x.beta, mu=mu, g=g,
...
updateZTS1 <- function(z, zg, y, lambda, x.beta,
                      phi, tau, taug, g, prec,
                      acc, att, mh, acc.phi, att.phi, mh.phi) {
  nt <- ncol(z)
  nknots <- nrow(z)
