...
\end{align}
where $\alpha \in \calR$ controls the skewness, $z_{t} \ind N_{(0, \infty)}(0, \sigma^2_t)$ are a random effect from a half-normal distribution (see appendix A.3), $v_t(\bs)$ is a Gaussian process with mean zero, variance one, \Matern correlation, and $\sigma^2_t \iid \text{IG}(a, b)$.
...
\end{align}
A proof of this can be found in Appendix A.2.
...
See Appendix A.1 for details regarding Gibbs sampling and Metropolis update rules.
...
\end{align}
The data from 5 July are shown in Figure \ref{fig:ozone} along with a Q-Q plot of the residuals compared to a skew-$t$ distribution with 8 d.f. and $\alpha = 1$.
\begin{center}
\begin{figure}
  \includegraphics[width=0.5\linewidth]{plots/ozone-5jul.pdf}
  \includegraphics[width=0.5\linewidth]{plots/qq-res.pdf}
  \caption{Ozone values on 5 July 2005 (left) Q-Q plot of the residuals (right)}
  \label{fig:ozone}
\end{figure}
\end{center}
...
\hl{Brian, I'm not sure that this is the best way to show this. Maybe a better heatplot would be quantile scores for $T=0$ for a few different quantiles. Then you can get a sense how the partitions impact the scores. The main problem is that $K=10, T=q(0.90)$ is much larger (5 - 7 times) than the rest of the quantile scores (across all values), so it really has an impact on the color scale}
