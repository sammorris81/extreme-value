...
\begin{align} \label{eq:fullmodel}
  Y_t(\bs) &= X_t(\bs) \beta + \alpha \sigma_t z_t + \sigma_t v_t(\bs)
\end{align}
where $\alpha \in \calR$ controls the skewness, $z_{t} \ind N_{(0, \infty)}(0, 1)$ are a random effect from a half-normal distribution (see appendix A.4), $v_t(\bs)$ is a Gaussian process with mean zero, variance one, \Matern correlation, and $\sigma^2_t \iid \text{IG}(a, b)$.
...
\subsection{Accounting for temporal dependence}
We account for temporal dependence through a AR(1) time series on the spatial knots and $z_t(\bs)$ terms.
These are modeled as
\begin{align}
  \bw_{t, k} | \bw_{t-1, k} &\sim N_2(\phi_w \bw_{t-1, k}, (1 - \phi_w^2) ) \\
  z_{t, k} | z_{t-1, k} &\sim N(\phi_z z_{t-1, k}, (1 - \phi_z^2))
\end{align}
where $|\phi_w|, |\phi_z| < 1$.
...
Let $\bw_{t1}, \ldots, \bw_{tK}$ be a set of daily spatial knots in a spatial domain of interest, $\calD$, so that
\begin{align*}
 P_{tk} = \{\bs : k = \argmin_\ell || \bs_t - \bw_{t\ell} ||\}.
\end{align*}
Because the spatial domain is bounded, we do a transformation on the knot locations to $\calR^2$.
Let $\Phi_2$ represent a bivariate standard normal density function, and let $\bw_k = \min(\bs) + \text{range}(\bs) * \Phi_2^{-1}(\bw^*_k)$.
...
\begin{align}
   ...
   \bw^*_{tk} \mid \bw^*_{t-1, k} &\sim N_2(\phi_w \bw^*_{t-1, k}, (1 - \phi_z^2))
\end{align}
...
If there is temporal dependence amongst the observations, then we update $z_tk$, using a Metropolis Hastings update.
