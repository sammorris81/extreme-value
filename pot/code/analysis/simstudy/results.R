...
for (setting in 1:nsettings) {
  ...
  plot(probs, bs.med.ref.gau[, 1, setting], type='o',
       ...
       ylim=c(ymin, ymax), main=paste("Data:", setting.title[setting]), ylab="relative brier score" , cex.lab=2, cex.axis=2, cex.main=2)
...
for (setting in 1:nsettings) {
  ...
  for (i in 2:(nmethods - 1)) {
    ...
    points(probs[-c(10,11)], qs.mean.ref.gau[-c(10,11), i, setting], pch=pch[i], col=col[i], bg=bg[i], cex=1.7)
    abline(h=1, lty=2)
  }
  if (setting == 6) {
    legend("topleft", legend=methods, lty=lty, col=col, pch=pch, pt.bg=bg, cex=1.6)
