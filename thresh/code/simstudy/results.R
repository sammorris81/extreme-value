...
# Lower grid for thresholds
load("scores-low.RData")
...
crps <- matrix(NA, nrow=3, ncol=10)

# integrate the brier scores
for (setting in 1:nsettings) {
  crps.set <- matrix(NA, nrow=10, ncol=nsets)
  for (set in 1:nsets) {
    brier.score.set <- grid.length * brier.scores[[setting]][[set]]
    crps.set[, set] <- colSums(brier.score.set) 
  }
