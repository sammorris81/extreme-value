...
Returning to the proposed parameterization (with $\bbeta = 0$), let $W^{-2} = \frac{b}{a}\sigma^{-2} \sim$ Gamma$(a / 2, a / 2)$ so that \eref{steq:fullmodel} in the manuscript becomes
\begin{align}
...
Because typical ozone concentration varies throughout the US, we have included Brier scores by site for two model fits (Gaussian and \Skewt, $K = 5$, $T = 50$) in \fref{stfig:bssite}.
...
\begin{figure}
  ...
  \caption{Map of Brier scores for Gaussian (top) vs \Skewt, $K = 5$, $T = 50$ (bottom).}
  \label{stfig:bssite}
\end{figure}
