...
The MCMC sampling for the model in Section \ref{s:hier} is done using {\tt R} (http://www.r-project.org). Whenever possible, we select conjugate priors (see Appendix \ref{a:posterior}); however, for some of the parameters, no conjugate prior distributions exist.
...
\section{Comparisons with other parameterizations} \label{a:otherparams}
...
To extend this to the skew-$t$ distribution, \citet{Azzalini2003} take $\tilde{Y}(\bs) = V\tilde{X}(\bs)$ where $V^{-2} \sim $ Gamma$(\nu / 2, \nu / 2)$. Returning to the proposed parameterization, let $\tau^{-2} = \frac{b}{a}\sigma^{-2} \sim$ Gamma$(a, a)$ so that (\ref{eq:fullmodel}) becomes
\begin{align}
  Y(\bs) = \tau \left[ \lambda \left(\frac{a}{b}\right)^{1 / 2} |z| + \left(\frac{a}{b}\right)^{1 / 2} v(\bs) \right].
\end{align}
Clearly setting $a = \nu / 2 > 0$, $b = \frac{\nu}{2(1 - \tilde{\lambda}^2)} > 0$, and $\lambda = \tilde{\lambda} / (1 - \tilde{\lambda}^2)^{1 / 2} \in (-\infty, \infty)$ resolves the difference in parameterizations.
We note that our parameterization has three parameters $(a, b, \lambda)$ compared to the to parameters of the alternative parameterization $(\nu, \tilde{\lambda})$.
However, if we introduce an overall scale parameter $c > 0$ into the alternative parameterization so that $\tilde{Y}(\bs) = c V \tilde{X}(\bs)$, then the two models remain equivalent by setting $a = \nu / 2$, $b = \frac{\nu}{2 c^2 (1 - \tilde{\lambda}^2)}$, and $\lambda = \tilde{\lambda} / (1 - \tilde{\lambda}^2)^{1 / 2}$.
