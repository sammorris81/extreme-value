...
\begin{align*}
    \pi(\beta \mid \ldots) & \propto \exp \left\{ - \frac{ 1 }{ 2 } \beta^\prime \Lambda_0 \beta - \frac{ 1 }{ 2 } \sum_{t = 1}^{n_t} [\bY_t - X_t \beta - \lambda |z_t|]^\prime \Omega [\bY_t - X_t \beta - \lambda |z_t|] \right\}\\
     & \propto \exp \left\{ -\frac{ 1 }{ 2 } \left[ \beta^\prime \Lambda_\beta \beta  - 2 \sum_{t = 1}^{n_t} [ \beta^\prime X_t^\prime \Omega (\bY_t - \lambda |z_t| )] \right] \right\}\\
     & \propto \mbox{N} ( \Lambda_\beta^{-1} \mu_\beta , \Lambda_\beta^{-1})
\end{align*}
where
\begin{align*}
    \mu_\beta &= \sum_{t = 1}^{n_t} \left[ X_t^\prime \Omega (\bY_t - \lambda |z_t|) \right]\\
    \Lambda_\beta &= \Lambda_0 + \sum_{t = 1}^{n_t} X_t^\prime \Omega X_t.
\end{align*}
...
\hl{TODO: Still needs to be updated for single $\lambda$}\\
...
Then
\begin{align*}
  \pi(\lambda_2 \mid \ldots) &\propto {\lambda_2^2}^{(-\alpha_\lambda - 1)} \exp\left\{ - \frac{ \beta_\lambda }{ \lambda_2^2} \right\} \prod_{t = 1}^{n_t} \prod_{k = 1}^K \frac{1} {\lambda_2} \exp\left\{ - \frac{ z^2_{tk} }{ 2 \lambda_2^2  \sigma_{tk})^2} \right\} \\
   & \propto {\lambda_2^2}^{(-\alpha_\lambda - K*n_t - 1)} \exp \left\{ -\frac{1}{\lambda^2_2} \left[ \beta_\lambda + \frac{ z^2 }{2 \sigma^2_{tk}} \right] \right\}
\end{align*}
Then $\lambda_2 \mid \ldots \sim IG \left(\alpha_\lambda + K * n_t, \beta_\lambda + \frac{ z^2 }{2 \sigma^2_{tk}} \right)$
...
Returning to the proposed parameterization (with $\bbeta = 0$), let $W^{-2} = \frac{2b}{a}\sigma^{-2} \sim$ Gamma$(a / 2, a / 2)$ so that \eref{steq:fullmodel} becomes
\begin{align}
  Y(\bs) = W \left[ \lambda \left(\frac{a}{2b}\right)^{1 / 2} |z| + \left(\frac{a}{2b}\right)^{1 / 2} v(\bs) \right].
\end{align}
Clearly setting $b = \frac{a}{(1 - \tilde{\lambda}^2)} > 0$, and $\lambda = \tilde{\lambda} / (1 - \tilde{\lambda}^2)^{1 / 2} \in (-\infty, \infty)$ resolves the difference in parameterizations.
