...
{\large Samuel A Morris\footnote[1]{North Carolina State University}, Brian J Reich\footnotemark[1]{}, Emeric Thibaud\footnote[2]{Colorado State University}, and Dan Cooley\footnotemark[2]{}}
...
We apply our method to an ozone analysis for July 2005, and find that our model does demonstrate some improvement over both Gaussian and max-stable methods in terms of predicting exceedances over a fixed threshold.

{\bf Key words}: Skew-$t$, random partition,
\end{abstract}
...
Because one of our goals is to model the dependence of the distribution in the tails of the data, we choose to censor values below a threshold.
Let
\beq\label{Yt}
...
Models were compared using cross validation with 100 sites used as training sites and 44 sites withheld for testing.
...
\begin{figure}
  ...
  \caption{Ozone values on 10 July 2005 (left) Q-Q plot of the residuals for a skew-$t$ distribution with 10 d.f. and $\alpha = 1$ (right)}
  \label{fig:ozone}
\end{figure}
...
Traditionally, a threshold is chosen based on the assumption that beyond the threshold, the data follow an extremal model.
In our example, a threshold of 75 ppb which is $q(0.92)$ for all observations, but marginally it represents anywhere from $q(0.06)$ to $q(1)$.
...
To ensure that the max-stable method runs in a reasonable amount of time, we take a stratified sample of 800 sites and consider this our new dataset.
