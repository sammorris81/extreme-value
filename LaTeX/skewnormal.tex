...
For a stationary and isotropic spatial process, the $\chi$ coefficient for two location $\bs$ and $\bt$ separated by distance $h = || \bs - \bt ||$ is
\begin{align}
...
In Figure \ref{fig:chi}, we give $\chi(h)$ for $K = 1, 3, 5, 10$ partitions for a skew-$t$ distribution with $\alpha = 10$, and $\nu = 3$ of freedom.
...
For each time-varying parameter, we transform to obtain a standard normal marginal distribution, place a Gaussian prior with autocorrelation of the transformed parameter, and then transform back to obtain the marginal distribution required to preserve the skew-$t$ process.
...
\end{align}
where IG is the distribution function of an inverse gamma random variable.
...
For designs 2 and 3, $\sigma^2_{tk} \iid \text{IG}(3, 8)$
For designs 1, we set $\lambda = 0$.
For designs 2 and 3, $\lambda = 3$ was used as to simulate moderate skewness, and the $z_t$ are generated as described in (\ref{eq:sitez}).
...
For data settings with skew-$t$ marginals (settings 2 -- 5), we find significant improvement over the Gaussian method.
...
\begin{figure}
  ...
  \label{fig:ozone}
\end{figure}
Exploratory data analysis indicates that there is dependence in the high quantile levels of the residuals beyond what we expect if the residuals were independent.
...
The upper thresholde of 75 ppb was used because the current air quality standard is based on exceedance of 75 ppb.
...
\hl{number of iterations, burnin, convergence}
...
We then compute the relative (to Gaussian) Brier scores to compare each model.

\subsection{Results}\label{s:results}
\hl{Include more here. 1: mention something about the max-stable results (pull from figure caption), 2: discuss general take home points}
