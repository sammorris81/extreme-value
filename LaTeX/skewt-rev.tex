...
\usepackage{xr}
\externaldocument{supplemental-rev}
...
In Section \ref{s:simstudy}, we present a simulation study that examines the predictive capabilities of this model compared to Gaussian and max-stable methods.
...
In this section we introduce a popular version \citep{Azzalini2014,Beranger2016}, and demonstrate how the connection between this parameterization and one that we propose to use.
\citet{Azzalini2014} and \citet{Beranger2016} define a skew-normal process as
\begin{align}\label{eq:origskewnorm}
  \tilde{X}(\bs) = \tilde{\lambda} |z| + (1 - \tilde{\lambda}^2)^{1 / 2} v(\bs), \quad \bs \in \calD
\end{align}
where $\tilde{\lambda} \in (-1, 1)$, $z \sim N(0, 1)$, $v(\bs)$ is a Gaussian process with mean zero, variance one, and spatial correlation function cor$(h)$ where $h = || \bs_1 - \bs_2 || \in \calR$ is the distance between sites $\bs_1$ and $\bs_2$, and $\calD \in \calR^2$ is the spatial domain of interest.
To extend the skew-normal process to a skew-$t$ process \citet{Azzalini2003} take
\begin{align} \label{eq:originalskewt}
  \tilde{Y}(\bs) = \xi + V X(\bs)
\end{align}
where $V^{-2} \sim$ Gamma$(\nu / 2, \nu / 2)$.
Now, let $\xi = \bX(\bs)^\prime \bbeta$ where $\bX(\bs)^\prime$ is a set of $p$ spatial covariates at site $\bs$, $\bbeta$ is the $p$-vector of regression parameters, $\sigma^{-2} = \frac{a}{b} V^{-2}$ where $a = \nu / 2$ and $b = \frac{\nu}{2 (1 - \tilde{\lambda}^2)} > 0$, and let $\lambda = \tilde{\lambda} / \sqrt{1 - \tilde{\lambda}^2} \in \calR$.
Then we can express (\ref{eq:originalskewt}) as
\begin{align}
  Y(\bs) = \bX(\bs)^\prime \bbeta + \lambda \sigma |z| + \sigma v(\bs)
\end{align}
where $\sigma^2 \sim $IG$(a, b)$.
...
\begin{align}
  \text{cor}(h) = (1 - \gamma) I(h = 0) + \gamma \frac{ 1 }{ \Gamma(\nu) 2^{ \nu - 1}} \left( \sqrt{2\nu} \frac{ h }{ \rho } \right)^{\nu} K_{\nu} \left( \sqrt{2\nu} \frac{ h }{ \rho } \right) \label{eq:matern}
\end{align}
...
We use a transformation of a Gaussian random variable on $\sigma^2_t(\bs)$ to ensure that the marginal distributions of $\sigma^2_t(\bs)$ are inverse gamma.
Let
\begin{align}
  \sigma^{2*}_t(\bs) =\Phi^{-1}\left\{ \text{IG}[\sigma^2_t(\bs)] \right\}
\end{align}
where IG is defined as before.
We also use a transformation of a Gaussian random variable on $z_{t}(\bs)$ to ensure that the marginal distributions of $z_t(\bs)$ are half-normal.
Let
\begin{align}
...
\begin{align}
  ...
  z^*_{tk} | z^*_{t-1, k} &\sim N \left[\phi_z z^*_{t-1, k}, (1 - \phi_z^2)\right] \\
...
Then
\begin{align}
   Y_t(\bs) \mid z_{t}(\bs), \sigma_t^2(\bs), P_{tk}, \Theta &= \bX_t(\bs)^T \beta + \lambda \sigma_t(\bs) |z_t(\bs)| + \sigma_t(\bs) v_t(\bs) \label{eq:hier}\\
