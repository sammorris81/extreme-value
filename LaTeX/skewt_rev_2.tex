...
\usepackage{xr}
\externaldocument{supplemental_rev_2}
...
Asymptotic dependence/independence (see \sref{sts:extdep}) are notions which describe the probability that two random variables simultaneously exceed of an extremely high level.
...
In \fref{stfig:chi}, we show $\chi(h)$ for $K = 1, 3, 5, 10$ partitions for a \skewt distribution with $\alpha = 10$, and 3 degrees of freedom.
...
We transform $z_{t}(\bs)$ to the Gaussian scale using the probability integral transform to ensure that the marginal distributions of $z_t(\bs)$ are half-normal.
Let
...
We also transform $\sigma^2_t(\bs)$ to the Gaussian scale using the probability integral transform to ensure that the marginal distributions of $\sigma^2_t(\bs)$ are inverse gamma.
Let
...
To impute the censored observations, at each MCMC iteration, for censored site $i$ and day $t$, we use the conditional distribution of $\widetilde{Y}_t(\bs_i)$ given the current imputed values for all censored sites and true values for all non-censored sites $\widetilde{Y}^{(c)}_t(\bs_{-i})$.
Conditional on $\widetilde{Y}^{(c)}_t(\bs_{-i})$, the imputation of $\widetilde{Y}_t(\bs_i)$ is done using a univariate truncated normal likelihood with mean and standard deviation from the conditional multivariate normal distribution of $\widetilde{Y}_t(\bs)$.
...
For the final design, we generated data from a Brown-Resnick max-stable process  using \texttt{rmaxstab} in the \texttt{SpatialExtremes} package of \texttt{R} \citep{Ribatet2015}.
...
\begin{enumerate}[leftmargin=3em]
  ...
  \item \citet{Reich2012} max-stable model thresholded at $T = q(0.80)$
\end{enumerate}
...
Each chain of the MCMC ran for 20,000 iterations which includes a burn-in period of 10,000 iterations.
...
For example, in \aref{sta:ozonespatdep}, we present a plot that suggests there is more agreement between sites that are close to one another than sites that are far from one another.
...
Each chain of the MCMC ran for 30,000 iterations which includes a burn-in period of 25,000 iterations.
...
See \aref{sta:ozonemodelperf} for the performance of the best two models for selected extreme quantiles.
...
Web Appendices, Tables, and Figures referenced in Sections 2.1, 2.2, 3.2, 3.3, 5.3, 6, and 6.2 are available with this paper at the Biometrics website on Wiley Online Library.
