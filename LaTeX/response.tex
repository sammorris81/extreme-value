...
\begin{enumerate}[1.]
  ...
  \begin{response}
    ...
    We have added clarification in Section \sref{sts:temporal} to clarify that the partition is created in the $\bw$ space as opposed to the $\bw^*$ space.
...
\begin{enumerate}[1.]
  ...
  \begin{response}
    ...
    Mapping the probability of extreme events could either be the marginal probability of an event, such as ozone exceeding 75ppb, or a map of an upper quantile, such as the map of the 99th percentile in Figure \fref{stfig:ozoneq99}.
  \end{response}
...
\begin{enumerate}[1.]
  ...
  \begin{response}
    As mentioned in \sref{sts:simdesign}, some of the designs for the simulation study match the methods used to fit the datasets (e.g. design 2 and method 2, design 3 and method 4).
    We have provided additional clarification in \sref{sts:simdesign} that for these models, we do appear to correctly estimate the skewness parameters and degrees of freedom.
...
\begin{enumerate}[1.]
  ...
  \begin{response}
    We now say throughout the proof in \aref{sta:proofsamepartition} that the point process is a homogeneous Poisson process with intensity $\mu(\cdot)$.
  \end{response}
\end{enumerate}
