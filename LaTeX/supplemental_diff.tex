...
%DIF DEL supplemental_orig.tex   Thu May 26 19:50:07 2016
%DIF ADD supplemental_rev.tex    Fri May 27 02:00:07 2016
...
The MCMC sampling for the model \DIFaddbegin \DIFadd{in Section }\DIFaddend \ref{s:hier} is done using {\tt R} (http://www.r-project.org). Whenever possible, we select conjugate priors (see Appendix \ref{a:posterior}); however, for some of the parameters, no conjugate prior distributions exist.
...
First, we generate a candidate for $\sigma^2_{tk}$ using an IG$(a^*/s, b^*/s)$ candidate distribution in an independence Metropolis-Hastings update where $a^* = (n_{tk} + 1) / 2 + a$, \DIFdelbegin \DIFdel{$b^* = [Y_{tk}^T \Sigma^{-1}_{tk} Y_{tk} + z_{tk}^2] / 2 + b$}\DIFdelend \DIFaddbegin \DIFadd{$b^* = [Y_{tk}^\prime \Sigma^{-1}_{tk} Y_{tk} + z_{tk}^2] / 2 + b$}\DIFaddend , $n_{tk}$ is the number of sites in partition $k$ on day $t$, and $Y_{tk}$ and $\Sigma^{-1}_{tk}$ are the observations and precision matrix for partition $k$ on day $t$.
The acceptance ratio is
\begin{align*}
  R = \left\{
    \frac{ l[Y_t(\bs) | {\sigma^2_{tk}}^{(c)}, \ldots] }{ l[Y_t(\bs) | {\sigma^2_{tk}}^{(r - 1)}]}
    \right\} \times \left\{
...
\begin{align*}
    \pi(z_l | \ldots) &\propto \exp \left\{ -\frac{ 1 }{ 2 } \left[
        \left( \begin{array}{c}
            ...
            R_2
        \end{array} \right)\DIFdelbegin \DIFdel{^T
        }\DIFdelend \DIFaddbegin \DIFadd{^\prime
...
where
\begin{align*}
    \mu_l &= \lambda ( R_1\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Omega_{11} + R_2\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Omega_{21} )\bOne\\
    \Lambda_l &= \lambda^2 \bOne\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Omega_{11} \bOne + \frac{ 1 }{ \sigma^2_l }.
\end{align*}
...
\begin{align*}
    \pi(\beta \mid \ldots) & \propto \exp \left\{ - \frac{ 1 }{ 2 } \beta\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Lambda_0 \beta - \frac{ 1 }{ 2 } \sum_{t = 1 }\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend [\bY_t - X_t \beta - \lambda |z_t|]\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Omega [\bY_t - X_t \beta - \lambda |z_t|] \right\}\\
     & \propto \exp \left\{ -\frac{ 1 }{ 2 } \left[ \beta\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Lambda_\beta \beta  - 2 \sum_{ t = 1 }\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend [ \beta\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend X_t\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Omega (\bY_t - \lambda |z_t| )] \right] \right\}\\
     & \propto \mbox{N} ( \Lambda_\beta^{-1} \mu_\beta , \Lambda_\beta^{-1})
\end{align*}
where
\begin{align*}
    \mu_\beta &= \sum_{t=1}^{T} \left[ X_t\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Omega (\bY_t - \lambda |z_t|) \right]\\
    \Lambda_\beta &= \Lambda_0 + \sum_{ t = 1 }^{ T} X_t\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Omega X_t.
\end{align*}
...
\begin{align*}
    \pi(\sigma^2 \mid \ldots) & \propto (\sigma^2)^{ -\alpha_0 - 1 / 2 - n / 2 - 1} \exp \left\{ -\frac{\beta_0}{\sigma^2} - \frac{ |z|^2 }{2 \sigma^2} - \DIFdelbegin \DIFdel{\frac{ (\bY - \bmu)^T \Sigma^{-1} (\bY - \bmu) }{2 \sigma^2} }\DIFdelend \DIFaddbegin \DIFadd{\frac{ (\bY - \bmu)^\prime \Sigma^{-1} (\bY - \bmu) }{2 \sigma^2} }\DIFaddend \right\} \\
    & \propto (\sigma^2)^{ -\alpha_0 - 1 / 2 - n / 2 - 1} \exp \left\{ - \frac{ 1 }{ \sigma^2 } \left[\beta_0 + \frac{ |z|^2 }{ 2 } + \frac{ 1 }{ 2 } (\bY - \bmu)\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Sigma^{-1} (\bY - \mu) \right] \right\} \\
    & \propto \mbox{IG} (\alpha^*, \beta^*)
\end{align*}
where
\begin{align*}
    \alpha^* &= \alpha_0 + \frac{1}{2} + \frac{n}{2} \\
    \beta^* &= \beta_0 + \frac{ |z|^2 }{ 2 } + \frac{ 1 }{ 2 }(\bY - \bmu)\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \Sigma^{-1} (\bY - \bmu).
\end{align*}
...
\DIFaddbegin \DIFadd{Let $c$ be the midpoint of $\bs_1$ and $\bs_2$.
...
Define }\DIFdelend \DIFaddbegin \DIFadd{over $A$ with intensity given by
  ...
  \DIFadd{\mu(A) = \lambda_{PP} |A| = \lambda_{PP} \pi \left(\frac{h}{2}\right)^2 = \lambda_{PPA}^* h^2.
}\end{align*}
...
Then
}\DIFdelend \DIFaddbegin \DIFadd{into four regions, $B_1$, $B_2$, $R_1$, $R_2$ as seen in Web Figure \ref{fig:hpp}.
}\begin{figure}
  \includegraphics[width=\linewidth]{plots/circles}
  \caption{\DIFaddFL{Illustration of the partition of $A$.}}
  \label{fig:hpp}
\end{figure}
\DIFadd{Let $N_j$ be the number of knots in $B_j, j = 1, 2$.
Then
}\begin{align}
  \DIFadd{P(\bs_1 \in P_i, \bs_2 \in P_{j \neq i}) \ge P(N_1 > 0, N_2 > 0)
}\end{align}
...
By definition of a Poission process, $N_1$ and $N_2$ are independent and thus $P(N_1 > 0, N_2 > 0) = P(N_1 > 0)^2$, and the intensity measure over $B_1$ is given by
  ...
  \DIFadd{\mu(B_1) }&\DIFadd{= \lambda_{PP} |B_1| = \lambda_{PP} \frac{h^2}{4} \left(\frac{2 \pi}{3} - \frac{\sqrt{3}}{2} \right) \nonumber }\\
       &\DIFadd{= \lambda^*_{PPB1} h^2.
}\end{align}
\DIFadd{So,
}\begin{align}
  \DIFadd{P(\bs_1 \in P_i, \bs_2 \in P_{j \neq i}) >= P(N_1 > 0)^2 = }[\DIFadd{1 - P(N_1 = 0)}]\DIFadd{^2 = }[\DIFadd{1 - \exp\left(-\lambda^*_{PPB1} h^2\right)}]\DIFadd{^2
}\end{align}
...
where
\begin{align}
  f_z(\bz) &= 2 t_d(\bz; \bar{\bOmega}, \eta) T \left[ \balpha\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \bz \sqrt{ \frac{\eta + d}{\nu + Q(\bz)} }; \eta + d\right] \\
  \bz &= \bomega^{-1}(\by - \xi)
\end{align}
where $t_d(\bz; \bar{\bOmega}, \eta)$ is a $d$-dimensional Student's $t$-distribution with scale matrix $\bar{\bOmega}$ and degrees of freedom $\eta$, \DIFdelbegin \DIFdel{$Q(z) = \bz^T \bar{\Omega}^{-1}\bz$ }\DIFdelend \DIFaddbegin \DIFadd{$Q(z) = \bz^\prime \bar{\Omega}^{-1}\bz$ }\DIFaddend and $T(\cdot; \eta)$ denotes the univariate Student's $t$ distribution function with $\eta$ degrees of freedom \citep{Azzalini2014}.

\subsection*{Extremal dependence}
For a bivariate skew-$t$ random variable \DIFdelbegin \DIFdel{$\bY = [Y(\bs), Y(\bt)]^T$}\DIFdelend \DIFaddbegin \DIFadd{$\bY = [Y(\bs), Y(\bt)]^\prime$}\DIFaddend , the $\chi(h)$ statistic \citep{Padoan2011} is given by
...
Consider the bivariate distribution of \DIFdelbegin \DIFdel{$\bY = [Y(\bs), Y(\bt)]^T$}\DIFdelend \DIFaddbegin \DIFadd{$\bY = [Y(\bs), Y(\bt)]^\prime$}\DIFaddend , with $\varrho(h)$ given by (\ref{eq:matern}).
...
\section{\DIFadd{Comparisons with other parameterizations}} \label{a:otherparams}
\DIFadd{Various forms of multivariate skew-normal and skew-$t$ distributions have been proposed in the literature.
In this section, we make a connection between our parameterization in (\ref{eq:fullmodel}) of the main text and another popular version.
\mbox{%DIFAUXCMD
...
\DIFadd{where $\tilde{\lambda} \in (-1, 1)$, $z ~ N(0, 1)$, $v(\bs)$ is a Gaussian process with mean zero, variance one, and spatial correlation function $\rho$.
...
Returning to the proposed parameterization, let $W^{-2} = \frac{2b}{a}\sigma^{-2} \sim$ Gamma$(a / 2, a / 2)$ so that (\ref{eq:fullmodel}) becomes
}\begin{align}
  \DIFadd{Y(\bs) = W \left[ \lambda \left(\frac{a}{2b}\right)^{1 / 2} |z| + \left(\frac{a}{2b}\right)^{1 / 2} v(\bs) \right].
}\end{align}
\DIFadd{Clearly setting $a = \nu / 2 > 0$, $b = \frac{\nu}{(1 - \tilde{\lambda}^2)} > 0$, and $\lambda = \tilde{\lambda} / (1 - \tilde{\lambda}^2)^{1 / 2} \in (-\infty, \infty)$ resolves the difference in parameterizations.
