...
%DIF DEL skewt_orig.tex   Thu May 26 19:50:07 2016
%DIF ADD skewt_rev.tex    Fri May 27 02:20:03 2016
...
\usepackage{xr}
%DIF 21c21
%DIF < \externaldocument{supplemental_orig}
%DIF -------
\externaldocument{supplemental_rev} %DIF > 
%DIF -------
...
The skew-elliptical family of distributions provides models that are mathematically tractable while introducing a slant parameter to account for asymmetric data\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Azzalini2014}}%DIFAUXCMD
}\DIFdelend .
A brief review of the additive process \DIFaddbegin \DIFadd{(\mbox{%DIFAUXCMD
\citealp{Azzalini2003}}%DIFAUXCMD
; \mbox{%DIFAUXCMD
\citealp[p. 129]{Azzalini2014}}%DIFAUXCMD
; \mbox{%DIFAUXCMD
\citealp{Beranger2016}}%DIFAUXCMD
) }\DIFaddend by which a skew-$t$ process is created is given here.
...
\begin{align}
  Y(\bs) = \bX(\bs)\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \bbeta + \lambda \sigma |z| + \sigma v(\bs) \DIFaddbegin \label{eq:fullmodel}
\DIFaddend \end{align}
where $\bX(\bs)$ is a set of spatial covariates at site $\bs$, $\bbeta$ is the vector of regression parameters, $\lambda \in \calR$ is a parameter controlling skew, $z \sim N(0, 1)$, \DIFdelbegin \DIFdel{$\sigma^2 \sim \text{IG}(a, b)$ }\DIFdelend \DIFaddbegin \DIFadd{$\sigma^2 \sim \text{IG}(a / 2, b)$ }\DIFaddend is random scale parameter, IG is the distribution function of an inverse gamma random variable, \DIFaddbegin \DIFadd{$a$ is the degrees of freedom, $b$ controls the scale, }\DIFaddend and $v(\bs)$ is a spatial Gaussian process with mean zero, variance one, and a positive definite correlation function \DIFaddbegin \DIFadd{$\rho(\bs_1, \bs_2)$.
Although any positive definite correlation function could be used, we choose to use the stationary isotropic }\Matern \DIFadd{correlation with
}\begin{align}
  \DIFadd{\rho(h) = (1 - \gamma) I(h = 0) + \gamma \frac{ 1 }{ \Gamma(\nu) 2^{ \nu - 1}} \left( \sqrt{2\nu} \frac{ h }{ \varphi } \right)^{\nu} K_{\nu} \left( \sqrt{2\nu} \frac{ h }{ \varphi } \right) \label{eq:matern}
}\end{align}
\DIFadd{where $\varphi$ is the spatial range, $\nu$ is the smoothness, $\gamma$ is the proportion of variance accounted for by the spatial variation, $K_\nu$ is a modified Bessel function of the second kind, and $h = || \bs_1 - \bs_2 ||$ is the distance between sites $\bs_1$ and $\bs_2$}\DIFaddend .

For a finite collection of locations $\bs_1, \ldots, \bs_n$, denote the vector of observations \DIFdelbegin \DIFdel{$\bY = [Y(\bs_1), \ldots, Y(\bs_n)]^T$}\DIFdelend \DIFaddbegin \DIFadd{$\bY = [Y(\bs_1), \ldots, Y(\bs_n)]^\prime$}\DIFaddend .
After marginalizing over both $z$ and $\sigma$, \DIFaddbegin \DIFadd{using the notation from \mbox{%DIFAUXCMD
\citet[p. 176]{Azzalini2014}}%DIFAUXCMD
,
}\DIFaddend \begin{align} \DIFaddbegin \label{eq:stazzalini}
  \DIFaddend \bY \sim \text{ST}_n(\bX \bbeta, \bOmega, \balpha, \DIFdelbegin \DIFdel{2a}\DIFdelend \DIFaddbegin \DIFadd{a}\DIFaddend ),
\DIFdelbegin %DIFDELCMD < %DIFDELCMD < \label{eq:fullmodel}%%%
%DIFDELCMD < %%%
\DIFdelend \end{align}
that is, $\bY$ follows an $n$-dimensional skew-$t$ distribution with location $\bX \bbeta$, \DIFdelbegin \DIFdel{correlation }\DIFdelend \DIFaddbegin \DIFadd{variance }\DIFaddend matrix $\bOmega$, slant parameters $\balpha$ and degrees of freedom \DIFdelbegin \DIFdel{$2a$, where $\bX = [\bX(\bs_1)^T, \ldots, \bX(\bs_n)^T]$}\DIFdelend \DIFaddbegin \DIFadd{$a$, where $\bX = [\bX(\bs_1)^\prime, \ldots, \bX(\bs_n)^\prime]$}\DIFaddend ,
$\bOmega = \bomega \bar{\bOmega} \bomega$,
\DIFdelbegin \DIFdel{$\bomega = \text{diag}\left(\frac{1}{ \sqrt{ab}}, \ldots, \frac{1}{\sqrt{ab}}\right)$,
$\bar{\bOmega} = (\bSigma + \lambda^2 \bOne \bOne^T$),
$\balpha = \lambda (1 + \lambda^2 \bOne^T \bSigma^{-1} \bOne)^{-1/2} \bOne^T \bSigma^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{$\bomega = \text{diag}\left(\sqrt{\frac{a(1 + \lambda^2)}{ b }}, \ldots, \sqrt{\frac{a(1 + \lambda^2)}{ b }} \right)$,
$\bar{\bOmega} = \frac{1}{1 + \lambda^2}(\bSigma + \lambda^2 \bOne \bOne^\prime)$,
$\balpha = \lambda (1 + \lambda^2)^{1 / 2} (1 + \lambda^2 \bOne^\prime \bSigma^{-1} \bOne)^{-1/2} \bSigma^{-1} \bOne$}\DIFaddend , and $\bSigma$ is the positive definite correlation matrix \DIFdelbegin \DIFdel{of $[v(\bs_1), \ldots, v(\bs_n)]$}\DIFdelend \DIFaddbegin \DIFadd{which is obtained from $\rho(h)$}\DIFaddend .
This process is desirable because of its flexible tail that is controlled by the skewness parameter $\lambda$ and degrees of freedom \DIFdelbegin \DIFdel{$2a$}\DIFdelend \DIFaddbegin \DIFadd{$a$}\DIFaddend .
Furthermore, the marginal distributions at each location also follow a univariate skew-$t$ distribution \citep{Azzalini2014}.
\DIFaddbegin \DIFadd{For comparisons with other parameterizations, please see \ref{a:otherparams}.
}\DIFaddend 


\DIFdelbegin \DIFdel{Although any positive definite correlation function could be used, we choose to use the stationary isotropic }%DIFDELCMD < \Matern %%%
\DIFdel{correlation with }\begin{eqnarray*}
  \DIFdel{\text{cor}[v(\bs_1), v(\bs_2)] = \gamma I(\bs_1 = \bs_2) + (1 - \gamma)  \frac{ 1 }{ \Gamma(\nu) 2^{ \nu - 1}} \left( \sqrt{2\nu} \frac{ h }{ \rho } \right)^{\nu} K_{\nu} \left( \sqrt{2\nu} \frac{ h }{ \rho } \right) %DIFDELCMD < \label{eq:matern}%%%
}\end{eqnarray*}
%DIFAUXCMD
...
}\DIFdelend %DIF >  We use this parameterization for spatial correlation because the $\gamma$ parameter permits the inclusion of a nugget effect to account for non-spatial variability due to issues like measurement error.
...
Let
\beq \DIFdelbegin %DIFDELCMD < %DIFDELCMD < \label{Yt}%%%
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \label{eq:Yt}
  \DIFaddend \widetilde{Y}(\bs) = \left\{ \begin{array}{ll}
...
\begin{align} \label{eq:partition}
  Y(\bs) &= \bX(\bs)\DIFdelbegin \DIFdel{^T }\DIFdelend \DIFaddbegin \DIFadd{^\prime }\DIFaddend \bbeta + \lambda \sigma(\bs) |z(\bs)| + \sigma(\bs) v(\bs).
\end{align}
...
\end{align}
\DIFdelbegin \DIFdel{All }\DIFdelend \DIFaddbegin \DIFadd{In other words, $P_1$ is composed of all sites for which the closest knot is $\bw_1$.
For all $\bs \in P_k$, with $k = 1, 2, \ldots, K$, the functions }\DIFaddend $z(\bs)$ and $\sigma(\bs)$ \DIFdelbegin \DIFdel{for sites in subregion $k$ are assigned common values
}\begin{eqnarray*}
  \DIFdel{z(\bs) = z_{k} \quad \text{and} \quad \sigma(\bs) = \sigma_{k} %DIFDELCMD < \label{eq:sitezsig}%%%
}\end{eqnarray*}
%DIFAUXCMD
\DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{are equal to the constants $z_k$ and $\sigma_k$ respectively.
and }\DIFaddend the $z_k$ and $\sigma^2_k$ are distributed as $z_k \iid N(0, 1)$ and $\sigma^2_k \iid$ IG$(a, b)$.
...
The partitioning model remove long-range dependence. Conditional on knots $\bw_1, \ldots, \bw_K$, the $\chi$ statistic for two sites \DIFdelbegin \DIFdel{$\bs$ and $\bt$ in partitions $k_s$ and $k_t$ respectively is
}\DIFdelend \DIFaddbegin \DIFadd{$\bs_1$ and $\bs_2$ in partitions $k_1$ and $k_2$ respectively is
}\DIFaddend \begin{align}
  \chi(h) &= I(k\DIFdelbegin \DIFdel{_s }\DIFdelend \DIFaddbegin \DIFadd{_1 }\DIFaddend = k\DIFdelbegin \DIFdel{_t}\DIFdelend \DIFaddbegin \DIFadd{_2}\DIFaddend ) \chi_{\text{skew-}t}(h) + I(k\DIFdelbegin \DIFdel{_s }\DIFdelend \DIFaddbegin \DIFadd{_1 }\DIFaddend \neq k\DIFdelbegin \DIFdel{_t}\DIFdelend \DIFaddbegin \DIFadd{_2}\DIFaddend ) \chi_{\text{Gaus}}(h) \nonumber \\
         &= I(k\DIFdelbegin \DIFdel{_s }\DIFdelend \DIFaddbegin \DIFadd{_1 }\DIFaddend = k\DIFdelbegin \DIFdel{_t}\DIFdelend \DIFaddbegin \DIFadd{_2}\DIFaddend ) \chi_{\text{skew-}t}(h) \DIFaddbegin \label{eq:chiskewt}
\DIFaddend \end{align}
...
Marginally, over the knots, $\chi(h) = \pi(h) \chi_{\text{skew}-t}(h)$, where \DIFdelbegin \DIFdel{$\pi(h) = \Pr(k_s = k_t)$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi(h) = \Pr(k_1 = k_2)$ }\DIFaddend is the probability that two sites separated by distance $h$ are in the same partition.
In \ref{a:proofsamepartition}, we show that $\lim_{h \rightarrow \infty} \pi(h) = 0$, \DIFdelbegin \DIFdel{implying }\DIFdelend \DIFaddbegin \DIFadd{assuming that the knots follow a homogeneous Poisson process.
This implies that }\DIFaddend $\lim_{h \rightarrow \infty} \chi(h) = 0$.
...
We use an AR(1) time series model for \DIFdelbegin \DIFdel{$w_{tk}$}\DIFdelend \DIFaddbegin \DIFadd{$\bw_{tk}$}\DIFaddend , $z_{tk}$, and $\sigma_{tk}$.
...
Let
\begin{align} \DIFaddbegin \label{eq:wstar}
  \DIFaddend w^*_{tki} = \Phi^{-1}\left[ \frac{ w_{tki} - \min(\bs_i)}{ \max(\bs_i) - \min(\bs_i) } \right], \quad i = 1, 2
\end{align}
...
We use a \DIFdelbegin \DIFdel{copula on $\sigma^2_t(\bs)$ }\DIFdelend \DIFaddbegin \DIFadd{transformation of a Gaussian random variable on $z_{t}(\bs)$ }\DIFaddend to ensure that the marginal distributions of \DIFdelbegin \DIFdel{$\sigma^2_t(\bs)$ are inverse gamma.
Let
}\DIFdelend \DIFaddbegin \DIFadd{$z_t(\bs)$ are half-normal.
Let
}\DIFaddend \begin{align} \DIFdelbegin \DIFdel{\sigma^{2*}}\DIFdelend \DIFaddbegin \label{eq:zstar}
  \DIFadd{z^*}\DIFaddend _t(\bs) = \Phi^{-1}\left\{ \DIFdelbegin \DIFdel{\text{IG}[\sigma^2_t(\bs)] }\DIFdelend \DIFaddbegin \DIFadd{\text{HN}[z_t(\bs)] }\DIFaddend \right\}
\end{align}
where \DIFdelbegin \DIFdel{IG is defined as before}\DIFdelend \DIFaddbegin \DIFadd{HN is the distribution function of a half-normal random variable}\DIFaddend .
We also use a \DIFdelbegin \DIFdel{copula on $z_{t}(\bs)$ }\DIFdelend \DIFaddbegin \DIFadd{transformation of a Gaussian random variable on $\sigma^2_t(\bs)$ }\DIFaddend to ensure that the marginal distributions of \DIFdelbegin \DIFdel{$z_t(\bs)$ are half-normal.
Let
}\DIFdelend \DIFaddbegin \DIFadd{$\sigma^2_t(\bs)$ are inverse gamma.
Let
}\DIFaddend \begin{align} \DIFdelbegin \DIFdel{z^*}\DIFdelend \DIFaddbegin \label{eq:sigmastar}
  \DIFadd{\sigma^{2*}}\DIFaddend _t(\bs) =\Phi^{-1}\left\{ \DIFdelbegin \DIFdel{\text{HN}[z_t(\bs)] }\DIFdelend \DIFaddbegin \DIFadd{\text{IG}[\sigma^2_t(\bs)] }\DIFaddend \right\}
\end{align}
where \DIFdelbegin \DIFdel{HN is the distribution function of a half-normal random variable}\DIFdelend \DIFaddbegin \DIFadd{IG is defined as before}\DIFaddend .
The AR(1) process for each tail parameter is $\bw^*_{1k} \sim N_w(0, 1)$, \DIFdelbegin \DIFdel{$z^*_{1k} \sim N(0, \sigma^2_{1k})$}\DIFdelend \DIFaddbegin \DIFadd{$z^*_{1k} \sim N(0, 1)$}\DIFaddend , $\sigma^{2*}_{1k} \sim N(0, 1)$, and for $t > 1$ the time series is modeled as
\begin{align}
  \bw^*_{tk} | \bw^*_{t-1, k} &\sim N_2\left[\phi_w \bw^*_{t-1, k}, (1 - \phi_w^2) \right] \DIFaddbegin \label{eq:ts-wstar}\DIFaddend \\
  z^*_{tk} | z^*_{t-1, k} &\sim N \left[\phi_z z^*_{t-1, k}, \DIFdelbegin \DIFdel{\sigma^2_{tk} }\DIFdelend (1 - \phi_z^2)\right] \DIFaddbegin \label{eq:ts-zstar} \DIFaddend \\
  \sigma^{2*}_{tk} | \sigma^{2*}_{t-1, k} &\sim N \left[\phi_\sigma \sigma^{2*}_{t-1, k}, (1 - \phi_\sigma^2) \right] \DIFaddbegin \label{eq:ts-sigmastar}
\DIFaddend \end{align}
where $|\phi_w|$, $|\phi_z|$, $|\phi_\sigma| < 1$.
These are stationary time series models with marginal distributions \hbox{$\bw^*_{k} \sim N_2(0, 1)$}, \DIFdelbegin %DIFDELCMD < \hbox{$z^*_{k} \sim N(0, \sigma^2_{k})$}%%%
\DIFdelend \DIFaddbegin \hbox{$z^*_{k} \sim N(0, 1)$}\DIFaddend , and \hbox{$\sigma^{2*}_{k} \sim N(0, 1)$}.
After transformation back to the original space, $\bw_{tk} \sim \text{Unif}(\calD)$, \DIFdelbegin \DIFdel{$z_{tk} \sim HN(0, \sigma^2_{tk})$ }\DIFdelend \DIFaddbegin \DIFadd{$z_{tk} \sim HN(0, 1)$, and }\DIFaddend $\sigma^2_{tk} \sim \text{IG}(a, b)$.
\DIFaddbegin \DIFadd{We then create the partition for day $t$ using $\bw_{t1}, \ldots, \bw_{tk}$.
}\DIFaddend For each day, the model is identical to the spatial-only model in (\ref{eq:partition}) by construction.

\section{Hierarchical model}\label{s:hier}
Conditioned on $z_{tk}(\bs)$, $\sigma^2_{tk}(\bs)$, and $P_{tk}$, the marginal distributions \DIFaddbegin \DIFadd{of $Y(\bs)$ }\DIFaddend are Gaussian and the joint distribution \DIFaddbegin \DIFadd{of $Y(\bs)$ is }\DIFaddend multivariate Gaussian.
...
\DIFdelbegin \DIFdel{In practice, we fix }\DIFdelend \DIFaddbegin \DIFadd{One approach would be to allow }\DIFaddend $K$ \DIFaddbegin \DIFadd{to be unknown and follow a Poisson process prior, but this would lead to onerous computing.
Therefore, we elect to treat $K$ as a tuning parameter for the MCMC by fixing it }\DIFaddend at different levels \DIFdelbegin \DIFdel{, and assess }\DIFdelend \DIFaddbegin \DIFadd{and assessing }\DIFaddend its impact on prediction as described in \ref{s:modelselect}.
Then
\begin{align}
   ...
   \lambda & \DIFdelbegin \DIFdel{= \lambda_1 \lambda_2}%DIFDELCMD < \nonumber\\
%DIFDELCMD <    %%%
\DIFdel{\lambda_1 }%DIFDELCMD < &%%%
\DIFdel{= }%DIFDELCMD < \left\{ \begin{array}{ll}
%DIFDELCMD <       +1 \quad & \text{w.p. } 0.5\\
%DIFDELCMD <       -1 \quad & \text{w.p. } 0.5
%DIFDELCMD <    \end{array}\right%%%
\DIFdel{.}%DIFDELCMD < \nonumber\\
%DIFDELCMD <    %%%
\DIFdel{\lambda^2_2 }%DIFDELCMD < & %%%
\DIFdelend \sim \DIFdelbegin \DIFdel{IG}\DIFdelend \DIFaddbegin \DIFadd{N}\DIFaddend (\DIFdelbegin \DIFdel{a}\DIFdelend \DIFaddbegin \DIFadd{0}\DIFaddend , \DIFdelbegin \DIFdel{b}\DIFdelend \DIFaddbegin \DIFadd{\sigma^2_\lambda}\DIFaddend )\nonumber\\
   %DIF >  \lambda_1 &= \left\{ \begin{array}{ll}
   %DIF >     +1 \quad & \text{w.p. } 0.5\\
   %DIF >     -1 \quad & \text{w.p. } 0.5
   %DIF >  \end{array}\right.\nonumber\\
   %DIF >  \lambda^2_2 & \sim IG(a, b)\nonumber\\
...
We generated data from 4 different simulation designs:
 \begin{enumerate} [leftmargin=3em]
  ...
  \DIFaddbegin \item \DIFadd{A Brown-Resnick process with range 1 and smoothness 0.5
}\DIFaddend  \end{enumerate} 
In the first three designs, the $v_t(\bs)$ terms were generated using a \Matern covariance with smoothness parameter $\nu = 0.5$ and spatial range \DIFdelbegin \DIFdel{$\rho = 1$}\DIFdelend \DIFaddbegin \DIFadd{$\varphi = 1$}\DIFaddend .
...
In designs 1 -- 3, the mean \DIFdelbegin \DIFdel{$\bX^T \bbeta = 10$ }\DIFdelend \DIFaddbegin \DIFadd{$\bX^\prime \bbeta = 10$ }\DIFaddend was assumed to be constant across space.
...
For Friedman tests that came back with a significant p-value, we conducted a Wilcoxon-Nemenyi-McDonald-Thompson \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Hollander2014} }%DIFAUXCMD
...
Initially, we fit a linear regression \DIFdelbegin \DIFdel{assuming a mean function of
  ...
  \DIFdel{\text{E}[Y_i(\bs)] = \beta_0 + \beta_1 \cdot \text{CMAQ}_t(\bs). %DIFDELCMD < \label{eq:datamean}%%%
}\end{eqnarray*}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{with $\bX_t(\bs) = [1, \text{CMAQ}_t(\bs)]^\prime$. %DIF >  assuming a mean function of
%DIF >  \begin{align}
%DIF >    \text{E}[Y_i(\bs)] = \beta_0 + \beta_1 \cdot \text{CMAQ}_t(\bs). \label{eq:datamean}
%DIF >  \end{align}
