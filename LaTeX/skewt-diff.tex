...
%DIF DEL skewt-orig.tex   Tue May 24 23:11:37 2016
%DIF ADD skewt-rev.tex    Wed May 25 00:57:51 2016
...
\usepackage{xr}
%DIF 21c21
%DIF < \externaldocument{supplemental-orig}
%DIF -------
\externaldocument{supplemental-rev} %DIF > 
%DIF -------
...
The spatial }\DIFdelend %DIF >  Let $Y(\bs)$ be the observation at spatial location $\bs$ in a spatial domain of interest $\calD \in \calR^2$.
%DIF >  The spatial skew-$t$ process can be written
%DIF >  \begin{align}
%DIF >    Y(\bs) = \bX(\bs)^T \bbeta + \lambda \sigma |z| + \sigma v(\bs)
%DIF >  \end{align}
%DIF >  where $\bX(\bs)$ is a set of spatial covariates at site $\bs$, $\bbeta$ is the vector of regression parameters, $\lambda \in \calR$ is a parameter controlling skew, $z \sim N(0, 1)$, $\sigma^2 \sim \text{IG}(a, b)$ is random scale parameter, IG is the distribution function of an inverse gamma random variable, and $v(\bs)$ is a spatial Gaussian process with mean zero, variance one, and a positive definite correlation function.
\DIFaddbegin 

%DIF >  For a finite collection of locations $\bs_1, \ldots, \bs_n$, denote the vector of observations $\bY = [Y(\bs_1), \ldots, Y(\bs_n)]^T$.
%DIF >  After marginalizing over both $z$ and $\sigma$,
%DIF >  \begin{align}
%DIF >    \bY \sim \text{ST}_n(\bX \bbeta, \bOmega, \balpha, 2a), \label{eq:fullmodel}
%DIF >  \end{align}
%DIF >  that is, $\bY$ follows an $n$-dimensional skew-$t$ distribution with location $\bX \bbeta$, correlation matrix $\bOmega$, slant parameters $\balpha$ and degrees of freedom $2a$, where $\bX = [\bX(\bs_1)^T, \ldots, \bX(\bs_n)^T]$, $\bOmega = \bomega \bar{\bOmega} \bomega$, $\bomega = \text{diag}\left(\frac{1}{ \sqrt{ab}}, \ldots, \frac{1}{\sqrt{ab}}\right)$, $\bar{\bOmega} = (\bSigma + \lambda^2 \bOne \bOne^T$), $\balpha = \lambda (1 + \lambda^2 \bOne^T \bSigma^{-1} \bOne)^{-1/2} \bOne^T \bSigma^{-1}$, and $\bSigma$ is the positive definite correlation matrix of $[v(\bs_1), \ldots, v(\bs_n)]$.
%DIF >  This process is desirable because of its flexible tail that is controlled by the skewness parameter $\lambda$ and degrees of freedom $2a$.
%DIF >  Furthermore, the marginal distributions at each location also follow a univariate skew-$t$ distribution \citep{Azzalini2014}.
\DIFadd{Various forms of multivariate skew-normal and }\DIFaddend skew-$t$ \DIFdelbegin \DIFdel{process can be written
}\begin{eqnarray*}
  \DIFdel{Y(\bs) = \bX(\bs)^T \bbeta + \lambda \sigma |z| + \sigma v(\bs)
}\end{eqnarray*}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{distributions have been proposed in the literature.
In this section we introduce a popular version \mbox{%DIFAUXCMD
\citep{Azzalini2014,Beranger2016}}%DIFAUXCMD
, and demonstrate how the connection between this parameterization and one that we propose to use.
\mbox{%DIFAUXCMD
\citet{Azzalini2014} }%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Beranger2016} }%DIFAUXCMD
define a skew-normal process as
}\begin{align}\DIFadd{\label{eq:origskewnorm}
  \tilde{X}(\bs) = \tilde{\lambda} |z| + (1 - \tilde{\lambda}^2)^{1 / 2} v(\bs), \quad \bs \in \calD
}\end{align}
\DIFaddend where \DIFdelbegin \DIFdel{$\bX(\bs)$ is a set of spatial covariates at site $\bs$, $\bbeta$ is the vector of regression parameters, $\lambda \in \calR$ is a parameter controlling skew}\DIFdelend \DIFaddbegin \DIFadd{$\tilde{\lambda} \in (-1, 1)$}\DIFaddend , $z \sim N(0, 1)$, \DIFdelbegin \DIFdel{$\sigma^2 \sim \text{IG}(a, b)$ is random scale parameter, IG is the distribution function of an inverse gamma random variable, and }\DIFdelend $v(\bs)$ is a \DIFdelbegin \DIFdel{spatial }\DIFdelend Gaussian process with mean zero, variance one, and \DIFdelbegin \DIFdel{a positive definite correlation function .
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{For a finite collection of locations $\bs_1, \ldots, \bs_n$, denote the vector of observations $\bY = [Y(\bs_1), \ldots, Y(\bs_n)]^T$.
After marginalizing over both $z$ and $\sigma$, }\begin{eqnarray*}
  \DIFdel{\bY \sim \text{ST}_n(\bX \bbeta, \bOmega, \balpha, 2a), %DIFDELCMD < \label{eq:fullmodel}%%%
}\end{eqnarray*}
%DIFAUXCMD
\DIFdel{that is, $\bY$ follows an $n$-dimensional }\DIFdelend \DIFaddbegin \DIFadd{spatial correlation function cor$(h)$ where $h = || \bs_1 - \bs_2 || \in \calR$ is the distance between sites $\bs_1$ and $\bs_2$, and $\calD \in \calR^2$ is the spatial domain of interest.
To extend the skew-normal process to a }\DIFaddend skew-$t$ \DIFdelbegin \DIFdel{distribution with location $\bX \bbeta$, correlation matrix $\bOmega$, slant parameters $\balpha$ and degrees of freedom $2a$, where $\bX = [\bX(\bs_1)^T, \ldots, \bX(\bs_n)^T]$}\DIFdelend \DIFaddbegin \DIFadd{process \mbox{%DIFAUXCMD
\citet{Azzalini2003} }%DIFAUXCMD
take
}\begin{align} \DIFadd{\label{eq:originalskewt}
  \tilde{Y}(\bs) = \xi + V X(\bs)
}\end{align}
\DIFadd{where $V^{-2} \sim$ Gamma$(\nu / 2, \nu / 2)$.
Now, let $\xi = \bX(\bs)^\prime \bbeta$ where $\bX(\bs)^\prime$ is a set of $p$ spatial covariates at site $\bs$}\DIFaddend , \DIFdelbegin \DIFdel{$\bOmega = \bomega \bar{\bOmega} \bomega$, $\bomega = \text{diag}\left(\frac{1}{ \sqrt{ab}}, \ldots, \frac{1}{\sqrt{ab}}\right)$, $\bar{\bOmega} = (\bSigma + \lambda^2 \bOne \bOne^T$), $\balpha = \lambda (1 + \lambda^2 \bOne^T \bSigma^{-1} \bOne)^{-1/2} \bOne^T \bSigma^{-1}$, and $\bSigma$ is the positive definite correlation matrix of $[v(\bs_1), \ldots, v(\bs_n)]$.
...
Furthermore, the marginal distributions at each location also follow a univariate skew-$t$ distribution \mbox{%DIFAUXCMD
\citep{Azzalini2014}}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{$\bbeta$ is the $p$-vector of regression parameters, $\sigma^{-2} = \frac{a}{b} V^{-2}$ where $a = \nu / 2$ and $b = \frac{\nu}{2 (1 - \tilde{\lambda}^2)} > 0$, and let $\lambda = \tilde{\lambda} / \sqrt{1 - \tilde{\lambda}^2} \in \calR$.
Then we can express (\ref{eq:originalskewt}) as
}\begin{align}
  \DIFadd{Y(\bs) = \bX(\bs)^\prime \bbeta + \lambda \sigma |z| + \sigma v(\bs)
}\end{align}
\DIFadd{where $\sigma^2 \sim $IG$(a, b)$.
}\DIFaddend 
...
\begin{align}
  \DIFdelbegin \DIFdel{\text{cor}[v(\bs_1), v(\bs_2)] }\DIFdelend \DIFaddbegin \DIFadd{\text{cor}(h) }\DIFaddend = \DIFaddbegin \DIFadd{(1 - }\DIFaddend \gamma\DIFaddbegin \DIFadd{) }\DIFaddend I(\DIFdelbegin %DIFDELCMD < \bs%%%
\DIFdel{_1 }\DIFdelend \DIFaddbegin \DIFadd{h }\DIFaddend = \DIFdelbegin %DIFDELCMD < \bs%%%
\DIFdel{_2}\DIFdelend \DIFaddbegin \DIFadd{0}\DIFaddend ) + \DIFdelbegin \DIFdel{(1 - }\DIFdelend \gamma \DIFdelbegin \DIFdel{)  }\DIFdelend \frac{ 1 }{ \Gamma(\nu) 2^{ \nu - 1}} \left( \sqrt{2\nu} \frac{ h }{ \rho } \right)^{\nu} K_{\nu} \left( \sqrt{2\nu} \frac{ h }{ \rho } \right) \label{eq:matern}
\end{align}
where $\rho$ is the spatial range, $\nu$ is the smoothness, $\gamma$ is the proportion of variance accounted for by the spatial variation, $K_\nu$ is a modified Bessel function of the second kind, and $h = || \bs_1 - \bs_2 ||$ \DIFaddbegin \DIFadd{is the distance between sites $\bs_1$ and $\bs_2$}\DIFaddend .
...
\begin{align}
  \chi(h) = \lim_{c \rightarrow c^*} \DIFdelbegin \DIFdel{\Pr[Y(\bs) > c | Y(\bt) > c]
...
\end{align}
\DIFaddbegin \DIFadd{In other words, $P_1$ is composed of all sites for which the closest knot is $\bw_1$.
}\DIFaddend All $z(\bs)$ and $\sigma(\bs)$ for sites in subregion $k$ are assigned common values
\begin{align}
...
The partitioning model remove long-range dependence. Conditional on knots $\bw_1, \ldots, \bw_K$, the $\chi$ statistic for two sites \DIFdelbegin \DIFdel{$\bs$ and $\bt$ in partitions $k_s$ and $k_t$ respectively is
  ...
  \chi(h) &= I(k\DIFdelbegin \DIFdel{_s }\DIFdelend \DIFaddbegin \DIFadd{_1 }\DIFaddend = k\DIFdelbegin \DIFdel{_t}\DIFdelend \DIFaddbegin \DIFadd{_2}\DIFaddend ) \chi_{\text{skew-}t}(h) + I(k\DIFdelbegin \DIFdel{_s }\DIFdelend \DIFaddbegin \DIFadd{_1 }\DIFaddend \neq k\DIFdelbegin \DIFdel{_t}\DIFdelend \DIFaddbegin \DIFadd{_2}\DIFaddend ) \chi_{\text{Gaus}}(h) \nonumber \\
         &= I(k\DIFdelbegin \DIFdel{_s }\DIFdelend \DIFaddbegin \DIFadd{_1 }\DIFaddend = k\DIFdelbegin \DIFdel{_t}\DIFdelend \DIFaddbegin \DIFadd{_2}\DIFaddend ) \chi_{\text{skew-}t}(h)
\end{align}
...
We use a \DIFdelbegin \DIFdel{copula }\DIFdelend \DIFaddbegin \DIFadd{transformation of a Gaussian random variable }\DIFaddend on $\sigma^2_t(\bs)$ to ensure that the marginal distributions of $\sigma^2_t(\bs)$ are inverse gamma.
Let
\begin{align}
  \sigma^{2*}_t(\bs) =\Phi^{-1}\left\{ \text{IG}[\sigma^2_t(\bs)] \right\}
\end{align}
where IG is defined as before.
We also use a \DIFdelbegin \DIFdel{copula }\DIFdelend \DIFaddbegin \DIFadd{transformation of a Gaussian random variable }\DIFaddend on $z_{t}(\bs)$ to ensure that the marginal distributions of $z_t(\bs)$ are half-normal.
Let
\begin{align}
...
\begin{align}
  ...
  z^*_{tk} | z^*_{t-1, k} &\sim N \left[\phi_z z^*_{t-1, k}, \DIFdelbegin \DIFdel{\sigma^2_{tk} }\DIFdelend (1 - \phi_z^2)\right] \\
