...
% \begin{center}
% {\Large {\bf A space-time \skewt model for threshold exceedances}}\\
...
We present a new space-time model for threshold exceedances based on the spatial \skewt process.
...
Our application with ozone data does not fit into this framework because we do not focus on exceedances of a very high level, and we have observations at over 1,000 ozone monitoring locations.

We propose a new spatiotemporal threshold exceedance model based on the \skewt process \citep{Padoan2011}.
We use a \skewt distribution because of its flexibility to model asymmetry and heavy-tailed data with the aim of predicting the probability of exceeding a high fixed level at an unobserved location.
...
In \sref{sts:spatial}, we build upon the traditional \skewt process by incorporating censoring to focus on tails, partitioning to remove long-range asymptotic dependence, and extending the model to space-time data.
...
We then compare our method to Gaussian and max-stable methods with a data analysis of ozone measurements from 1,000 sites throughout the US in \sref{sts:analysis}.
...
\end{align}
that is, $\bY$ follows an $n$-dimensional \skewt distribution with location $\bX \bbeta \in \calR^n$; covariance matrix $\bOmega_{n \times n} = \bomega \bar{\bOmega} \bomega$ where \mbox{$\bar{\bOmega}_{n \times n} = \frac{1}{1 + \lambda^2}(\bSigma + \lambda^2 \bOne \bOne^\top)$}, $\bSigma_{n \times n}$ is the positive definite correlation matrix which is obtained from $\rho(h)$, and $\bomega_{n \times n} = \text{diag}\left(\sqrt{\frac{ b }{a(1 + \lambda^2)}}, \ldots, \sqrt{\frac{ b }{a(1 + \lambda^2)}} \right)$; slant parameters $\balpha \in \calR^n = \lambda (1 + \lambda^2)^{1 / 2} (1 + \lambda^2 \bOne^\top \bSigma^{-1} \bOne)^{-1/2} \bSigma^{-1} \bOne$, and degrees of freedom $a$.
Furthermore, the marginal distributions at each location also follow a univariate \skewt distribution \citep{Azzalini2014}.
This process is desirable because of its flexible tail that is controlled by the skewness parameter and degrees of freedom.
...
For the \skewt distribution, $c^* = \infty$.
...
As mentioned previously, we propose to use a censored approach because we do not want the low-to-moderate values to influence the fit of the overall model.
...
\beq \label{steq:Yt}
  ...
  \end{array} \right.
\eeq
be the censored process at site $\bs$ where $Y(\bs)$ is the uncensored process, $\delta(\bs) = I[Y(\bs) > T]$, and $T$ is a pre-specified threshold value.
...
Across partitions, the $Y(\bs)$ remain dependence via the correlation function for $v(\bs)$ because $v(\bs)$ spans all partitions.
...
In \fref{stfig:chi}, we give $\chi(h)$ for $K = 1, 3, 5, 10$ partitions for a \skewt distribution with $\alpha = 10$, and 3 degrees of freedom.
...
However, we choose to allow for temporal dependence in the $\bw$, $z$, and $\sigma$ terms because these terms dictate the tail behavior which is our primary focus (see \aref{sta:temporal}).
Let
...
These are stationary time series models with marginal distributions \hbox{$\bw^*_{k} \sim N_2(\bZero, \bI_2)$}, where $\bI_2 = \text{diag}(1, 1)$, \hbox{$z^*_{k} \sim N(0, 1)$}, and \hbox{$\sigma^{2*}_{k} \sim N(0, 1)$}.
...
For each day, the model is identical to the spatial-only model in \eref{steq:partition} by construction.\citep{
...
We use MCMC methods to explore the posterior distribution of the model parameters.
...
\begin{enumerate}[leftmargin=3em]
  ...
  \item Multivariate asymmetric logistic distribution \citep{Stephenson2003} %with GEV(1, 1, 0.2) marginals with dependence parameteter $\gamma_{\text{AL}} = 0.5$
...
For designs 2 and 3, $\lambda = 3$ was used as to simulate moderate skewness, and the $z_t$ were generated as described in Section \sref{sts:part}.
In designs 1 -- 3, the mean $\bX^\top \bbeta = 10$ was assumed to be constant across space.
In the fourth design, we generated from a multivariate asymmetric logistic distribution \citep{Stephenson2003}.
...
For this setting, we set the dependence parameter $\gamma_{\text{AL}} = 0.5$ which gives moderate spatial dependence.
For the asymmetry parameters, we use standardized Gaussian as in \citet{Reich2012} with bandwidth 1.
The dependence parameter $\gamma_{\text{AL}} \in (0, 1)$, is similar to $\gamma$ in that it controls the relative strength of the nugget effect.
For the final design, we generate data from a Brown-Resnick max-stable process \citep{Kabluchko2009} using \texttt{rmaxtab} in the \texttt{SpatialExtremes} package of \texttt{R} \citep{Ribatet2015}.
For this design we give choose unit \Frechet margins, and use a range of 1 with smoothness 0.5.
...
Finally, we do not fit a Brown-Resnick model because we cannot use a Bayesian approach with so many sites.
...
% \end{align}
\fref{stfig:ozone-qq} shows a Q-Q plot of the residuals compared to a \skewt distribution with $a = 10$ and $\lambda = 1$.
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{plots/qq-res.pdf}
  \caption{Gaussian Q-Q plot (left) and \skewt distribution with $a = 10$ and $\lambda = 1$ Q-Q plot (right) for of the residuals.}
