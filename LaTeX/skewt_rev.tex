...
We impute the censored values as a step in the MCMC algorithm used to fit the model described in \sref{sts:comp}, and use cross-validation to select $T$.
...
For all $\bs \in P_k$, with $k = 1, 2, \ldots, K$, the functions $z(\bs)$ and $\sigma(\bs)$ are equal to the constants $z_k$ and $\sigma_k$ respectively, and the $z_k$ and $\sigma^2_k$ are distributed as $z_k \iid N(0, 1)$ and $\sigma^2_k \iid$ IG$(a, b)$.
...
\end{align}
where $I(\cdot)$ is an indicator function, $\chi_{\text{skew-}t}(h)$ is the $\chi$ statistic for a skew-$t$ process given in \eref{steq:chiskew-t}, $\chi_{\text{Gaus}}(h)$ is the $\chi$ statistic for a Gaussian process, and $h = ||\bs_1 - \bs_2||$.
...
For methods that correspond directly to a simulation design (e.g. design 2 and method 2, design 3 and method 4), $\lambda$ and $a$ appear to converge to their true values.
...
In this case, the max-stable method outperforms our method for very extreme quantiles; however, for predictions at high quantile levels, the differences between the max-stable method and our method decrease.
